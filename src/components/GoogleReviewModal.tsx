...
export default function GoogleReviewModal() {
  ...
  return (
    <div onClick={close} style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(8,12,22,.72)", backdropFilter:"blur(3px)", display:"flex", alignItems:"center", justifyContent:"center", padding:"1.2rem" }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:"380px", background:"#1a2236", border:"1px solid rgba(234,107,20,.35)", borderRadius:"16px", padding:"1.8rem 1.5rem 1.5rem", boxShadow:"0 24px 70px rgba(0,0,0,.55)", textAlign:"center", fontFamily:"'DM Sans',sans-serif", color:"#f0f4ff", position:"relative" }}>
        ...
        <h3 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"1.55rem", letterSpacing:".02em", margin:"0 0 .5rem", color:"#f0f4ff" }}>{copy.title}</h3>
        <p style={{ fontSize:".92rem", lineHeight:1.5, color:"rgba(208,219,242,.85)", margin:"0 0 1.4rem" }}>{copy.body}</p>
        ...
        <div style={{ display:"flex", gap:".5rem", justifyContent:"center" }}>
          <button onClick={close} style={{ flex:1, padding:".6rem", background:"rgba(255,255,255,.06)", color:"rgba(208,219,242,.85)", border:"1px solid rgba(255,255,255,.12)", borderRadius:"9px", fontFamily:"inherit", fontSize:".82rem", cursor:"pointer" }}>Maybe later</button>
          <button onClick={never} style={{ flex:1, padding:".6rem", background:"none", color:"rgba(190,205,235,.5)", border:"1px solid rgba(255,255,255,.08)", borderRadius:"9px", fontFamily:"inherit", fontSize:".82rem", cursor:"pointer" }}>Don't ask again</button>
        </div>
      </div>
    </div>
  );
