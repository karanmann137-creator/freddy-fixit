...
export default function ForContractors() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: "var(--ff-bg)", color: "var(--ff-text)", minHeight: "100vh" }}>
      ...
      <style>{".fc-cta{display:inline-flex;align-items:center;gap:.5rem;background:#ea6b14;color:#fff;border:none;font-family:'Bebas Neue',sans-serif;letter-spacing:.06em;font-size:1.15rem;padding:.85rem 2.1rem;border-radius:10px;cursor:pointer;transition:transform .15s,box-shadow .15s;box-shadow:0 8px 26px rgba(234,107,20,.28)} .fc-cta:hover{transform:translateY(-2px);box-shadow:0 12px 32px rgba(234,107,20,.4)} .fc-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(240px, 100%), 1fr));gap:1rem} .fc-card{background:rgba(var(--ff-fg), .04);border:1px solid rgba(var(--ff-fg), .08);border-radius:12px;padding:1.6rem 1.4rem} .fc-steps{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(210px, 100%), 1fr));gap:1rem} .fc-faq{border-bottom:1px solid rgba(var(--ff-fg), .08);padding:1.1rem 0} .fc-faq h3{color:var(--ff-text);font-size:1.05rem;margin-bottom:.4rem;font-family:'Bebas Neue',sans-serif;letter-spacing:.04em} .fc-p{line-height:1.75;color:rgba(var(--ff-muted), .85);font-weight:300} .fc-h{font-family:'Bebas Neue',sans-serif;letter-spacing:.05em}"}</style>

      <section style={{ maxWidth: "880px", margin: "0 auto", padding: "6rem clamp(1rem, 4vw, 1.5rem) 2.5rem", textAlign: "center" }}>
