...
- **`Ic` has no `menu`, `repeat`, `info` or `file` glyph.** Substitutes in use: inline `<span>` bars for hamburger, `refresh` for recurring, `bell` for info, `download` for documents. Check the icon set before naming one — a missing glyph renders **blank**, not an error. **There is no icon library installed** — `lucide-react` was a dependency for months without a single import and was removed on 2026-08-23 (39MB), along with `sonner`. Add the glyph to `Ic`; do not reach for a package.
...
**MONEY:** touches none of the four payout guards. It only decides whether `client_fee` is 0 on somebody's first job, and it makes waivers rarer, never more common; health check 2 already tolerates a 0 fee. **Known gap:** `ClientOnboarding.tsx` and `AuthCallback.tsx` call `apply_referral_code` fire-and-forget and discard the reason, so a retired or in-use code typed at signup fails silently (pre-existing behaviour for `invalid` too). The dashboard entry box is the second chance that does explain.
...
**Ordering is load-bearing.** `form.workType` must be answered on step 2 because `wt` drives both the credential copy on step 4 and the per-document `required` flags on step 5. `TOTAL`, the three 5-element arrays (`STEP_TITLES`, `STEP_SUBS`, `CONTRACTOR_GUIDE`, all indexed `[step-1]`) and every `if (step === N)` in `validate()` move together — and the draft-restore clamp `d.step <= TOTAL` is what makes a stale 8-step draft land on step 1 instead of a blank screen.
...
`GuideBubble.tsx` gives each step a plain-language reframe + a "Why we ask:" line, wired into both onboarding flows and `NewRequest`.
