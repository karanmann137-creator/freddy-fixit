...
- **`tools/ship.py`'s deletion gate needles differ by KIND of file, and the two kinds fail in opposite directions (2026-09-17).** A TS/TSX import omits the extension (`from "../components/GuideBubble"`), so only the bare **stem** can match it. An asset is referenced by URL and always carries the extension (`src="/onboarding-videos/x.mp4"`), so a bare-stem needle there is needlessly loose — it refused `public/icons.svg` because the words `text/icons/borders` appear in a **prose comment** in `Home.tsx`. `check_deletions` now picks the needle off the extension. **A gate that cries wolf is a gate someone learns to skip**, which is how a real dangling import ships — and a dangling import is not always a build error, since a lazy-loaded page fails only at runtime, in the browser, for whoever visits that route. Both directions are regression-tested by hand before trusting a run: the served `public/` mp4 must still be REFUSED (`ClientOnboarding.tsx`), a live component must still be REFUSED (`MessagesInbox.tsx` → both dashboards), and only the comment-matched asset may newly pass.
- **The other half of that gate is `_public_twin()`, and it is md5-gated on purpose.** Vite serves **only** `public/`, so a URL-style reference can never reach an identically-named file outside it — without the exemption a byte-identical 5MB duplicate at the repo root is pinned in the public repo forever by a reference belonging to the copy that actually serves. The waiver is granted **only** on an md5 match, so "same name, different file" is never waived. That distinction was load-bearing here: `onboarding-shots/` and `onboarding-video/shots/` carry the same seven filenames and are **different renders**, so they had to be justified as scratch, not as duplicates.
...
- **A block gated only on a `*_at` flag lingers forever, because nothing clears those flags on completion.** The contractor's "the client changed the time" prompt was gated on `client_rescheduled_at && !reschedule_accepted_at` alone, so it and its Accept button sat on completed and cancelled jobs indefinitely; it now also requires `status in (assigned, scheduled, in_progress)`. (`propose_job_schedule` **does** null both flags when a pro counters, so the gate only stops terminal-state leakage and cannot strand a live negotiation.) `RequestPhotoQuote`'s `canQuote` was likewise a bare `true` and stayed editable after completion. When adding a flag-gated block, ask what clears the flag — usually nothing does.
  ...
  **That fix had been applied to one of TWO copies, and the second was found 2026-09-16.** The reschedule prompt exists twice on the contractor side: the panel (gated, and carrying the comment explaining why) and the **"Needs your attention" row**, which had no status gate at all. So after a job completed the pro's attention list still read *"… changed the time on 'X'"* forever, its **Review new time** button did nothing visible — `A_RESCHED` only renders on the live statuses, so `focusJobAnchor` fell through to `scrollTo({top:0})` — and it burned one of only **four** attention slots, which is the money-gating rows' budget. **When you gate a condition, grep for every other place that reads the same flags**; a documented fix applied to one of two call sites is indistinguishable from no fix on the other.

- **The client rating form was unreachable, and `reviews` = 0 against a completed, released, receipted job is how it was found (2026-09-17).** Nothing refused the review: `submit_review` carries the null-safe guard, has **no time window**, and the job was still reviewable in principle. The defect was pure **reachability**, assembled from four separately-reasonable decisions in `ClientDashboard`. The form renders only when the completed job is the **active** job; `openReqs` filters `completed` out; `activeReq` falls back to `requests[0]` **only when `openReqs` is empty**; the request switcher lists `openReqs` alone. So History's **View** button was the one and only thing on the platform that could make a finished job active — and it was hidden on `r.status !== "completed"`, i.e. **on exactly the status the form needs**. Any client with a second open request could never reach it at all.

  The ask then survived solely as the one-shot `requestCompletionThanks` modal fired at the confirm moment, so **miss it or dismiss it and the rating was gone forever** — and the review is the last step of the funnel, the thing `get_homepage_reviews` renders as social proof and the only input to a pro's rating. The fix is one predicate (`cancelled` only) plus the label **"View & rate"**, because "View" gives a client no reason to press it. `cancelled` stays hidden — nothing to do on it. Safe because the job read at `activeReq.id` already excludes only *cancelled* jobs, so a completed request loads its job normally, and the form self-gates on `activeJob.status === "completed"` **and** `hasReviewed` (which renders the "thanks for rating" state instead, so the button never lies to someone who already rated).

  **The general shape: a form that is correct, granted and gated correctly can still be unreachable, and nothing anywhere reports it.** No error, no empty state, no failing check — just a table that stays at zero. When a feature's table has 0 rows against non-zero opportunities, suspect the route to it before suspecting the RPC.

- **`pickBid` dropped the job read's error, re-opening at a second call site the exact hole `jobLoadFailed` exists to close (2026-09-16).** `ClientDashboard`'s main load effect handles this correctly; `pickBid` did not. After `accept_bid` succeeded the bids were cleared two lines later, so a flaky read left `activeJob === null` and **nothing replaced them** — no agreement panel, no pay button, no banner, on the single screen where a client has just committed to a pro and is about to pay. It now sets `setJobLoadFailed(!!jobErr)` and only assigns the job on success, so the existing banner fires. (The `setRequests` patch below it already falls back to `r.assigned_contractor_id`, so the pick itself survives the failed read.) **A failed read is not an empty result — and the rule has to hold at every call site, not just the one where it was first written.**
...
## ⚠️ The null-unsafe guard was a FAMILY of twenty, and nineteen of them were anon-reachable (2026-09-16)

`set_job_autopay` was found by probe, then the three dispute-path RPCs above. Both notes called the shape latent. **It was not latent.** Enumerating every `public` function that is `SECURITY DEFINER`, matches `<>\s*auth\.uid\(\)`, and is executable by `anon` or by PUBLIC returned **twenty** functions — and unlike the three above, these are reachable with the **publicly-shipped anon key**, which is a valid project-signed JWT sitting in the JS bundle. Nineteen needed the fix; the twentieth (`place_bid`) turned out to be self-closing.

**`SECURITY DEFINER` is what removed RLS as the backstop, and it is the half that makes this reachable rather than theoretical.** The guard reads like it has two layers:

```sql
select client_id, contractor_id into v_client, v_contractor from jobs where id = p_job_id;
if v_client is null then raise exception 'Job not found'; end if;   -- null-SAFE, but…
if v_contractor <> auth.uid() then raise exception 'Not authorized'; end if;  -- NULL ⇒ never fires
```

The lookup runs **as the definer**, so it finds the row and populates `v_client` even for an anon caller — the "not found" raise therefore does not fire either — and the mutation afterwards also bypasses RLS. Two guards, both passed, nothing left standing.

**Demonstrated live, not inferred.** An MCP SQL session has no JWT, so `auth.uid()` is NULL there — **the MCP session IS the unauthenticated case**, which turns the standard rolled-back probe into a direct exploit demonstration. Before the fix: `set_job_checklist -> SUCCEEDED with NULL auth.uid(); checklist changed = true` and `update_client_request -> SUCCEEDED with NULL auth.uid()`. After: all seven tested refuse with `Not authorized` (`set_job_checklist`, `update_client_request`, `contractor_on_my_way`, `mark_job_complete`, `approve_job_schedule`, `submit_review`, `withdraw_job`).

Sharpest of the nineteen, in order: **`approve_job_schedule`** (the client's approval step immediately preceding payment), **`propose_price_change`** / **`request_quote_revision`** (a stranger altering a job's price), **`mark_job_complete`** (starts the 3-day auto-confirm clock), **`submit_review`** (fabricated reviews on any completed job), **`decline_price_reopen`** (cancels the job, declines the winning bid and re-opens every other bid — though its three money guards still stand). `withdraw_job` is the mildest because `job_money_block(p_job_id)` runs immediately after the broken guard. The rest: `approve_walkthrough`, `complete_walkthrough`, `confirm_visit`, `contractor_accept_reschedule`, `contractor_on_my_way`, `decline_price_change`, `decline_walkthrough`, `propose_job_schedule`, `propose_walkthrough`, `set_job_checklist`, `update_client_request` (guard variable is **`v_owner`**, not `v_client` — match on the shape, not the name), `update_job`.

**`place_bid` needs no change, and why is worth remembering.** Its guard is a compound reservation test, not this shape — but more to the point, `select status into v_active from contractors where id = auth.uid()` leaves `v_active` NULL for an anon caller, and the very next guard is `if v_active is null then raise 'Only contractors can bid'`. **An `is null` test is null-SAFE, so it fires.** `bids.contractor_id` is NOT NULL as a second backstop. **Read the adjacent guard before assuming a broken one is reachable.**

**All nineteen were patched programmatically, and that is what makes byte-for-byte preservation structural rather than a claim.** A `do` block loops `pg_get_functiondef(oid)` — which re-emits `prosrc` verbatim inside an auto-chosen dollar quote and preserves `SET search_path` — applies one `regexp_replace` inserting `auth.uid() is null or `, then **refuses to apply unless the byte delta is exactly 22** (`length('auth.uid() is null or ')`) and refuses to finish unless exactly 19 functions were patched. `CREATE OR REPLACE` preserves owner and ACLs, and no signature changes, so PostgREST sees no new overload. Hand-transcribing twenty bodies was the obvious approach and the wrong one. Migration `20260916_null_safe_auth_uid_guards.sql`.

**MONEY: touches none of the four payout guards.** Every guard is made **stricter** — it refuses a caller it previously accepted and accepts nobody new — which is the safe direction. `platform_health_check()` unchanged after apply.

**Two detection predicates to reuse, because both have bitten:**

- True-PUBLIC executability is `proacl is null OR (proacl)[1]::text like '=X/%'`. **`proacl::text like '%=X/%'` is WRONG** — it matches ordinary role grants like `authenticated=X/postgres` and returned ~190 mostly-false rows.
- The `search_path` backlog is **hygiene, not a bug.** Roughly a hundred functions lack a pinned `search_path`, but filtering to those that actually call pgcrypto **unqualified** — the only thing that raises 42883 — returns **zero**. Do not mass-apply a fix to a hundred live definitions for a problem none of them has.
...
`my_bid_threads()` returns threads + unread + snippet in one call. Its client side is one row per pro who bid on any of my requests, so a thread can be **started** from the list; its contractor side returns only threads the client has already opened, so an empty result genuinely means nobody has written. `mark_bid_thread_read(request, contractor)` returns early unless the caller is one of the two parties.

**⚠️ The two branches were asymmetric, and that asymmetry was the bug (2026-09-16).** The client branch filtered `r.status = 'pending'`; the contractor branch had **no status filter at all**. So the moment a client picked somebody, every bid conversation vanished from their side — while the losing pros kept their threads and went on replying into a conversation the client could no longer see. Fixed with one added predicate: a request that has left `'pending'` still yields any thread that **actually has messages** (`or exists (select 1 from messages m2 …)`), so picking a pro doesn't leave six dead rows in the inbox either. Deliberately an `or exists` on the existing client branch rather than a **third UNION branch** — a new branch would need its own `limit 1` subselect for `bid_amount`, which could disagree with the join-derived `b.amount` and defeat the UNION's dedup, producing two rows for one thread. Migration `20260916_my_bid_threads_keep_started_threads_after_pick.sql`.
...
**Bid threads now live in the MESSAGES INBOX on both dashboards, not just on a card (2026-09-16).** The reported bug was "the contractors have no way of replying to a chat when a client messages before picking someone", and it was five faults stacked, only one of which was the DB asymmetry above. The Messages tab rendered `my_conversations()` alone — which is **job-scoped**, and at bid stage there is no job — so a bid thread had **no home in the inbox at all**; the pro's only entry point was a button nested inside one Available Jobs card, which disappears the moment they hide the job or the request leaves `list_open_jobs()`; bid threads had no realtime; and the sidebar Messages badge excluded bid unread, so nothing anywhere told a pro a client had written.

**`src/lib/bidThreads.ts` is the bid-side twin of `chatUnread.ts`** — `useBidThreads(userId)` returns `{threads, started, byKey, byRequest, totalUnread, loading, error, refresh, markRead, unreadFor}`, with a realtime channel on `messages` INSERT plus a `CHAT_CHANGED_EVENT` listener, **both debounced 400ms** so a burst is one refetch. A failed read sets `error` and **keeps the previous list** — a failed read is not an empty result.

Three things about the merge are load-bearing:

- **The synthetic row key is `bid:<request>:<pro>`, and `job_id` deliberately holds that same non-uuid string.** `conversationKey(c)` in `chatUnread.ts` is the one key function for both kinds. Putting the synthetic key in `job_id` means any job-scoped path that mistakenly receives a bid row fails **loudly** with a bad-uuid error rather than silently marking the wrong thread read.
- **`openConversation` branches on `c.kind === "bid"` FIRST**, before any job lookup, for the same reason.
- **Loading and error compose differently, because they mean different things.** `inboxLoading = (convLoading || bid.loading) && inboxRows.length === 0` (show rows the moment either call answers), but `inboxError = convError && bid.error` — `MessagesInbox` renders its error state *instead of* the list, so one failing call must not hide the other's rows. Its error branch returns EARLY for that reason.

`BidChat.send()` now calls `announceChatChange()` after a successful insert. The realtime channel covers the **recipient's** dashboard, but the **sender's own** inbox row sitting behind the open drawer would keep a stale snippet and timestamp until something else refreshed it.
...
**Cadence is biweekly, not weekly (2026-09-24).** The two cron schedules above are unchanged and still fire every Tue/Thu — pg_cron has no native "every N weeks" — so `kick_newsletter()` gates internally instead: `v_weeks := (date_trunc('week', now())::date - '2000-01-03'::date) / 7`, and it returns early on odd `v_weeks` (same early-return shape as the `outbound_paused()` check right above it). Anchoring both audiences to the same fixed Monday epoch is what keeps them synchronized — contractor (Tue) and client (Thu) always fall in the same calendar week, so they can never drift onto opposite weeks. Verified by exhaustive `generate_series` over Sept–Nov 2026: clean 14-day alternation, both audiences always agree on which week is "on."
...
`GuideBubble.tsx` (the Freddy speech-bubble reframe + "Why we ask:" line) was **removed from both onboarding flows on 2026-08-24** in favour of `OnboardingProgress.tsx` — a numbered step bar — plus a single short `STEP_TITLES[step-1]` line. The step count is now carried visually by the bar, so nothing textual repeats it.

**The file itself was DELETED on 2026-09-17, and this line is why it needed checking twice.** This paragraph said GuideBubble "is still used on `NewRequest`" right up until the 2026-09-09 returning-client pass took it out of there too — so the doc claimed a live mount that no longer existed, and a filename grep appeared to agree because `OnboardingProgress.tsx` names it in a comment. **An import-graph walk from `src/main.tsx` is the authority on reachability, not a grep and not this file**; it put GuideBubble in a set of exactly two unreachable `src/` files (the other being `src/CLAUDE.md`, which is documentation). When a component is retired from its last mount, delete it in the same pass — a dead file that a stale doc vouches for is worse than either alone.
...
**own-Page-only** (Meta ToS forbids scanning others' posts or groups) — reads FB Page comments (`{page}/feed`) and DMs (`{page}/conversations`) plus IG comments (`{ig}/media`), classifies each against `CLIENT_ASK`/`CONTRACTOR_ASK`, dedupes into `social_leads` (`MAX_LEADS_PER_RUN = 12`, `MAX_AGE_HOURS = 48`) and emails an approval digest to hello@. It **drafts** replies and never sends one. All three secrets are set — `META_PAGE_TOKEN`, `META_PAGE_ID`, and **`META_IG_ID` = `17841418373715145` (2026-09-04)**; App **Freddy Lead Scout** `1580205976489384` (Development mode, which is sufficient for a Page you administer), Page **FreddyFixit** `1339030759289723`, six scopes: `pages_show_list`, `pages_messaging`, `pages_read_engagement`, `pages_read_user_content`, `instagram_basic`, `instagram_manage_comments`.

**The Instagram half went live 2026-09-04.** `freddy.fixit` (a professional account, 194 followers) was connected to the Page through Meta Business Suite — Graph returned the same id for **both** `instagram_business_account` and `connected_instagram_account`, which is what distinguishes a genuine professional link from a loose "connected account" that the `{ig}/media` endpoint would refuse. The **"Allow access to Instagram messages in Inbox" toggle was deliberately left ON**: IG DM access is exactly what the scout reads, and turning it off blinds that half silently. Verified by a token-gated `{"test":true}` run: `scanStatus` = `{feed: HTTP 200, conversations: HTTP 200, media: HTTP 200}`, `alerted:"none"`. **A missing `META_IG_ID` is invisible rather than an error** — `scanIgComments()` returns `[]` early, so `media` simply never appears in `scanStatus` and nothing says why. Read `scanStatus` for three keys, not two.
