...
export default function ClientDashboard() {
  ...
  // A failed read is not an empty result. Without this a flaky request looked
  // identical to "no job yet" and the ENTIRE payment surface vanished silently.
  const [jobLoadFailed, setJobLoadFailed] = useState(false);
  // Set when Stripe redirects back here: "success" | "cancelled".
  ...
  // create-payment-intent / create-balance-payment / create-milestone-payment /
  // create-recurring-prepayment all send the client to
  //   /client-dashboard?payment=success|cancelled&job=<id>   (or &tab=recurring)
  // and until now NOTHING in src/ read those params. A client who had just paid
  // landed on a dashboard that still said "Pay now", with no confirmation of any
  // kind — the single most alarming moment to leave silent.
  //
  // Declared BEFORE the deep-link effect on purpose: that one calls
  // clearDashNavFromUrl(), which strips ?job=, and this needs to read it first.
  //
  // The webhook is what actually flips the job to 'held' and it can land a few
  // seconds after the redirect, so success re-reads the job a few times rather
  // than trusting the first paint.
  useEffect(() => {
    const q = new URLSearchParams(window.location.search);
    const p = q.get("payment");
    const paidJobId = q.get("job");
    if (p !== "success" && p !== "cancelled") return;
    setPayReturn(p);
    // Strip it so a later refresh can't re-show a stale banner. Same
    // history.replaceState approach as clearDashNavFromUrl.
    try {
      const u = new URL(window.location.href);
      u.searchParams.delete("payment");
      const qs = u.searchParams.toString();
      window.history.replaceState({}, "", u.pathname + (qs ? "?" + qs : "") + u.hash);
    } catch { /* the banner still shows; only the URL is untidy */ }
    if (p !== "success" || !paidJobId) return;
    let stop = false;
    (async () => {
      for (const ms of [2500, 4000, 7000]) {
        await new Promise(r => setTimeout(r, ms));
        if (stop) return;
        try {
          const { data, error } = await supabase.from("jobs").select("*").eq("id", paidJobId).maybeSingle();
          if (stop || error || !data) continue;
          // Never clobber a different job the client has since switched to.
          setActiveJob((j: any) => (j && j.id !== data.id) ? j : data);
          if (data.payment_status === "held" || data.payment_status === "released") return;
        } catch { /* keep trying — the next pass may land */ }
      }
    })();
    return () => { stop = true; };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // The job read's error used to be thrown away, so a network blip rendered
      // exactly like "this request has no job" — every payment control, the
      // confirm button and the contract panel all disappeared with no
      // explanation. Keep the last-known job on screen and say so instead.
      if (jobErr) {
  ...
  const confirmCompletion = async () => {
    ...
    if (activeJob.payment_status === "held") {
      ...
      try {
        ...
        // invoke() throws the response body away, so the 409's plain-English
        // reason is only reachable off error.context.
        let reason: string | null = (data && (data as any).error) || null;
        if (error) {
          reason = error.message;
          try { const b = await (error as any)?.context?.json?.(); if (b?.error) reason = b.error; } catch { /* keep the fallback */ }
        }
        if (reason) {
          // This used to report EVERY failure as green "being processed… nothing
          // more for you to do", which is flatly wrong for the two failures that
          // actually happen. An unpaid balance can ONLY be fixed by the client,
  ...
  const s = {
    ...
    header: { background:"var(--ff-bg)", borderBottom:"1px solid rgba(var(--ff-fg), .07)", padding:".75rem 1.5rem", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap" as const, gap:".75rem", position:"sticky" as const, top:"3.75rem", zIndex:5, boxShadow:"0 4px 14px rgba(0,0,0,.12)" },
  ...
  // instantly, before the profile exists). Point them back to finish, not a blank page.
  if (!profile) return (
    <div style={{ ...s.wrap, display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      <div style={{ textAlign:"center", maxWidth:"380px", padding:"1.5rem" }}>
        ...
        <div style={{ fontSize:".92rem", color:"rgba(var(--ff-muted), .75)", lineHeight:1.55, marginBottom:"1.25rem" }}>
          Your login is set up, but we need a couple more details before you can post a request and get estimates. It takes about two minutes.
        </div>
        <button style={s.primaryBtn} onClick={() => setLocation("/client-onboarding")}>Finish setting up →</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={s.wrap} className="ffdash">
      ...
      <div style={s.content}>
        ...
        {/* Stripe sends the client straight back here. Until this existed the four
            checkout functions redirected to `/client`, which is not a route, so
            every paying client landed on the marketing homepage with no
            confirmation that their money had arrived. */}
        {payReturn === "success" && (
          <div style={{ margin:"0 0 1.25rem", padding:"1rem 1.1rem", borderRadius:"12px", background:"rgba(34,197,94,.1)", border:"1px solid rgba(34,197,94,.45)", display:"flex", alignItems:"flex-start", gap:".6rem" }}>
            <Ic name="check" size={16} color="#22c55e" style={{ marginTop:2, flexShrink:0 }} />
            <div style={{ flex:"1 1 auto", minWidth:0 }}>
              <div style={{ fontSize:".88rem", fontWeight:700, color:"#22c55e", marginBottom:".25rem" }}>Payment received</div>
              <div style={{ fontSize:".82rem", color:"rgba(var(--ff-muted), .8)", lineHeight:1.5 }}>
                Your money is <strong>held safely</strong> and only released to your contractor once you confirm the work is done. It can take a few seconds to show below.
              </div>
            </div>
            <button onClick={() => setPayReturn(null)} aria-label="Dismiss" style={{ background:"none", border:"none", color:"rgb(var(--ff-muted))", fontSize:"1.2rem", lineHeight:1, cursor:"pointer", padding:"0 .2rem" }}>&times;</button>
          </div>
        )}
        {payReturn === "cancelled" && (
          <div style={{ margin:"0 0 1.25rem", padding:"1rem 1.1rem", borderRadius:"12px", background:"rgba(234,107,20,.1)", border:"1px solid rgba(234,107,20,.45)", display:"flex", alignItems:"flex-start", gap:".6rem" }}>
            <Ic name="alert-triangle" size={16} color="#ea6b14" style={{ marginTop:2, flexShrink:0 }} />
            <div style={{ flex:"1 1 auto", minWidth:0 }}>
              <div style={{ fontSize:".88rem", fontWeight:700, color:"#ea6b14", marginBottom:".25rem" }}>Payment not completed</div>
              <div style={{ fontSize:".82rem", color:"rgba(var(--ff-muted), .8)", lineHeight:1.5 }}>
                You weren't charged. Your job is still here — use the payment button on it whenever you're ready.
              </div>
            </div>
            <button onClick={() => setPayReturn(null)} aria-label="Dismiss" style={{ background:"none", border:"none", color:"rgb(var(--ff-muted))", fontSize:"1.2rem", lineHeight:1, cursor:"pointer", padding:"0 .2rem" }}>&times;</button>
          </div>
        ...
        {/* A failed read is not an empty result. The last-known job stays on
            screen behind this rather than the whole payment surface vanishing. */}
        {jobLoadFailed && (
          <div style={{ margin:"0 0 1.25rem", padding:"1rem 1.1rem", borderRadius:"12px", background:"rgba(239,68,68,.08)", border:"1px solid rgba(239,68,68,.4)", display:"flex", alignItems:"flex-start", gap:".6rem" }}>
            <Ic name="alert-triangle" size={16} color="#ef4444" style={{ marginTop:2, flexShrink:0 }} />
            <div style={{ fontSize:".82rem", color:"rgba(var(--ff-muted), .85)", lineHeight:1.5 }}>
              <strong style={{ color:"var(--ff-text)" }}>We couldn't load the latest on this job.</strong> What you see below may be out of date — please refresh. Nothing has changed with your job or your money.
            </div>
          </div>
        )}
                          ...
                          <>
                            ...
                            {(activeJob.payment_status === "held" || activeJob.payment_status === "released") ? (
                              ...
                              <>
                                ...
                                {/* The button is deliberately NOT disabled while processing — a
                                    client who closed the Stripe tab would otherwise be stranded
                                    for two hours with no way to pay. */}
                                {activeJob.payment_status === "processing" && (
                                  <div style={{ fontSize:".82rem", color:"var(--ff-warn)", marginBottom:".6rem", lineHeight:1.5 }}><Ic name="alert-triangle" size={13} style={{ marginRight:4 }} />You have a checkout already open. If you've just paid, give it a moment and refresh. If you closed it without paying, start a new one below — <strong>don't pay in both</strong>.</div>
                                )}
                          ...
                          <div {...anchor("ffc-confirm")} style={anchorPad}>
                            ...
                            {(activeJob.payment_status === "held" || activeJob.payment_status === "released") ? (
                              ...
                              <>
                                ...
                                {depositSplit(activeJob) && <PaySplit job={activeJob} />}
                                {/* This branch never had a failed/processing notice, so a client
                                    whose card was declined after the work was done saw a plain pay
                                    button and no reason it hadn't worked. */}
                                {activeJob.payment_status === "failed" && (
                                  <div style={{ fontSize:".82rem", color:"var(--ff-danger)", marginBottom:".6rem", lineHeight:1.5 }}><Ic name="alert-triangle" size={13} style={{ marginRight:4 }} />Your last payment didn't go through. No charge was made — please try again below.</div>
                                )}
                                {activeJob.payment_status === "processing" && (
