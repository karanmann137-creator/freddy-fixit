...
## Onboarding
`ContractorOnboarding.tsx`, 8 steps: basics → specialties → service area → availability → trade → credentials → photo → documents. **The only submit path is the final "Complete Registration"** — an earlier "sign up now, finish later" fast-track was removed because it produced accounts with almost nothing.
...
## Profile completion
`contractorGaps(c)` in ContractorDashboard returns `{key, label, anchor}[]`; `contractorMissing(c)` is its `.map(g => g.label)`. `ContractorProfileCompletion.tsx` exports `GAP_ANCHORS` (`cpc-area` / `cpc-worktype` / `cpc-credentials` / `cpc-docs`) and takes a `highlight` prop.
...
- **Requests** — bids render read-only at **every** status (they used to vanish the moment a client picked). The accepted bid gets a green `✓ PICKED` chip; declined bids dim. **The client picks bids** — the admin assign dropdown and accept-bid buttons were removed.
