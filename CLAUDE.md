...
- **`Ic` has no `menu`, `repeat`, `info` or `file` glyph.** Substitutes in use: inline `<span>` bars for hamburger, `refresh` for recurring, `bell` for info, `download` for documents. Check the icon set before naming one — a missing glyph renders **blank**, not an error.
