...
// Contractor-facing "Request help" — sends a support message to the Freddy team
...
export default function RequestHelpModal({
  ...
  const submit = async () => {
    if (!message.trim()) { setErr("Please describe what you need help with."); return; }
    ...
    try {
      const { error } = await supabase.functions.invoke("support-request", {
        body: {
          role: "contractor",
          user_id: userId,
          subject: subject.trim() || "Contractor support request",
        ...
        {sent ? (
          ...
          <>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <div style={{ fontSize: ".72rem", color: "rgba(var(--ff-muted), .5)", marginTop: ".3rem" }}>Find the Job ID on the job card if your question is about a specific job.</div>
            </div>

            <div style={{ marginBottom: "1rem" }}>
              <label style={label}>Message *</label>
              <textarea value={message} rows={4} placeholder="Describe the issue or question…" onChange={e => setMessage(e.target.value)} style={{ ...field, resize: "vertical" as const }} />
            </div>
