...
**pg_cron jobs:** `daily-reminders` (16:30 UTC → `run_reminders()`), `reconcile-payouts` (every 15 min), `platform-health-check` (15:00 UTC), `visit-reminders` (every 10 min), `release_unconfirmed_visits` (every 15 min), `newsletter-contractor` (Tue 16:00), `newsletter-client` (Thu 16:00), `prune-rate-limits` (04:17 UTC daily → `prune_rate_limit_hits()`), plus auto-confirm. **As of 2026-09-01 the site is `open` and all 13 cron jobs are armed.** (This line previously said the site was in `waitlist` with six stood down — that was stale. Read `platform_settings.mode` / `outbound_paused()` rather than trusting a doc.) `set_platform_mode('paused'|'waitlist')` stands six of them down and `set_platform_mode('open')` re-arms them; the real guarantee is `outbound_paused()` **inside** each emitter, not the cron list. DB→edge calls go through `net.http_post` with the anon bearer, wrapped in an exception guard (pattern: `kick_reconcile_payouts()`).
